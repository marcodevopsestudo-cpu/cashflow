...
/// <summary>
/// Unit tests for <see cref="GetDailyBalanceFunction"/>.
/// </summary>
public sealed class GetDailyBalanceFunctionTests
{
    /// <summary>
    /// Ensures that the function returns the daily balance when it exists.
    /// </summary>
    [Fact]
    public async Task Run_ShouldReturnOkWithDailyBalancePayload_WhenDailyBalanceExists()
    {
        // Arrange
        ...
        mediator
            .Setup(x => x.Send(It.IsAny<GetDailyBalanceQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(dailyBalance);

        var function = new GetDailyBalanceFunction(mediator.Object);
        var context = new FakeFunctionContext();
        var request = new FakeHttpRequestData(context, HttpMethod.Get.Method);

        // Act
        var response = await function.Run(request, date, CancellationToken.None);
        ...
        var payload = await JsonSerializer.DeserializeAsync<DailyBalanceDto>(
            response.Body,
            new JsonSerializerOptions(JsonSerializerDefaults.Web));

        payload.Should().NotBeNull();
        payload!.BalanceDate.Should().Be(parsedDate);
        payload.TotalCredits.Should().Be(1500m);
        payload.TotalDebits.Should().Be(500m);
        payload.Balance.Should().Be(1000m);
        payload.UpdatedAtUtc.Should().Be(updatedAtUtc);

        mediator.Verify(
            x => x.Send(
                It.Is<GetDailyBalanceQuery>(q => q.Date == parsedDate),
    ...
    internal sealed class FakeHttpRequestData : HttpRequestData
    {
            ...
            : base(functionContext)
        {
            Headers = [];
            Method = method;
            Url = new Uri("https://localhost/api/daily-balance");
            Cookies = [];
            Identities = [];
        }
